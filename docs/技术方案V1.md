...
约束：experience FAQ 必须有 `experience_id`；general FAQ 必须没有 `experience_id`。JobTrack 通过 source Interview 推导，不在 FAQ 中重复保存。
...
3. Experience FAQ 缺 Experience 时不能提交。
4. General FAQ 不允许携带 Experience。
5. 批量提交要么全部写入，要么全部回滚。
