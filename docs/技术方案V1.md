...
### 2.3 面试支持 Deadline 与固定时段

Interview 与 Assessment 一样可以保存 Deadline 或固定时段。固定时段保存 `start_at` 与 `end_at`，创建时结束默认开始后一小时且用户能够修改；Deadline 只保存 `deadline_at`，作为点事项进入工作台和日历，不参与时段冲突。

### 2.4 FAQ 出现次数延期

一期没有母问题归并流程时，单条 FAQ 的出现次数没有稳定语义。当前只展示可选的来源面试，不做跨 FAQ 聚合；完成 CanonicalQuestion 归并后再展示聚合出现次数。
...
对象默认私有，客户端通过需要当前会话的 `/api/assets/[id]` 下载，不暴露存储凭据或对象 key。
...
Resume 不进行就地文件替换；新文件创建新 Resume。交互式上传要求同时关联至少一项已有 Experience，或快速创建至少一项 Experience。
...
约束：deadline 型必须且只能有 `deadline_at`；fixed_slot 型必须且只能有合法的 `start_at < end_at`。历史面试在迁移时保留原开始/结束时间并默认为 fixed_slot。
...
| 存在未来 fixed_slot Interview，或尚未确认发生的 deadline Interview | `action_required` |
| 存在有效、未完成且未取消的用户 Task | `action_required` |
| 以上均不存在 | `waiting` |

固定时段 Interview 结束后不再使 JobTrack 保持 `action_required`；未确认发生的 deadline Interview 在截止后继续要求行动并标记逾期，确认发生后才进入等待和待复盘判断。
...
| 标记 | 推导条件 |
|---|---|
| `overdue` | 未完成 Task、Assessment 或未确认发生的 deadline Interview 的截止时间早于 now |
...
固定时段 Interview 未取消、`end_at` 不晚于 now 且 `reviewed_at` 为空时，推导为 `review_due`；deadline Interview 还必须已经确认发生。复盘截止为对应结束/截止时间后三小时。它由工作台单独聚合；固定时段的待复盘状态本身不证明面试已经实际发生。
...
| `delete_assessment` | 彻底删除 Assessment 及关联 Task；用于清理误录记录，不创建 Timeline 事实 |
...
| `delete_interview` | 彻底删除 Interview、准备 Task、来源 FAQ 与 Transcript 文件；用于清理误录记录 |
...
### 7.5 Knowledge services

| Service function | 主要联动 |
|---|---|
| `createExperience` / `updateExperience` / `deleteExperience` | 维护 Experience；仍被 FAQ 引用时阻止删除 |
| `setResumeExperiences` | 原子替换一份 Resume 的 Experience 关联 |
| `parseFaqBlocks` | 按 `---` 拆分并逐 Block 返回解析结果，不写库 |
| `commitFaqBatch` | 校验绑定状态、分类和 Experience；来源面试可选；有关联面试时必要地确认发生；批量写入 FAQ |
| `updateFaq` / `deleteFaq` | 修改或删除 FAQ；绑定状态决定派生类型 |
| `createFaqCategory` / `renameFaqCategory` / `deleteFaqCategory` | 管理未绑定 FAQ 分类；重命名同步历史引用，删除后历史 FAQ 回到待补充 |

## 8. 错误模型

业务层当前使用稳定的错误前缀，Server Action 负责映射成用户提示，并把内部异常记录为脱敏摘要：
...
同一 `idempotencyKey` 重放不会报错，而是直接返回先前保存的结果。
...
| `/faq` | 问题/经历视图、筛选、分类管理 | getKnowledgeLibrary、Knowledge services |
| `/experiences/[id]` | 经历详情 | getExperienceDetail |
| `/interviews/[id]` | 面试准备与复盘 | getInterviewKnowledgeDetail |
| `/quick` | 快捷记录完整页 | quick option queries、JobWorkflow |
| `/login` | 邮箱密码登录 | Better Auth |
| `/api/health` | 认证配置、数据库、对象存储就绪检查 | readiness checks |
| `/api/export` | 当前用户结构化 JSON 导出 | exportUserData |
| `/api/assets/[id]` | 当前用户私有文件下载 | private object storage |

全局快捷操作以浮动按钮打开 modal；`/quick` 保留可直接访问的完整页，但不占一级导航。移动端使用底部一级导航，桌面端使用侧边导航，路由和读取模型一致。
...
3. 文件上传前校验类型和大小；JD 图片单张不超过 5MB、一次最多 4 张，Resume 和 Transcript 单文件不超过 10MB。
4. Resume、JD 图片和 Transcript 默认私有，通过会话保护的同源 Route Handler 访问，并设置 `private, no-store` 等私有响应头。
...
6. 批量提交要么全部写入，要么全部回滚。
7. 自定义分类重命名同步历史 FAQ；删除分类只清空分类引用，不删除 FAQ。
...
```text
src/
├── app/                              # App Router 页面、Server Actions 与 Route Handlers
│   ├── api/                          # auth、assets、export、health
│   ├── jobs/                         # 岗位列表、详情与业务操作
│   ├── interviews/                   # 面试准备与复盘
│   ├── faq/                          # FAQ / Experience 知识库
│   ├── experiences/                  # Experience 详情
│   ├── calendar/                     # 周日历
│   ├── quick/                        # 快捷记录页
│   └── login/                        # 账号密码登录
├── components/                       # 工作区壳、全局快捷操作、通用 modal
├── modules/
│   ├── job-workflow/
│   │   ├── interface.ts
│   │   ├── implementation.ts
...
│       └── derive-*.ts
├── adapters/
│   ├── database/                     # PostgreSQL JobWorkflow store
│   └── storage/                      # S3-compatible private storage
├── db/                               # schema、client、runtime
└── shared/
    ├── actor/                        # 当前用户与认证模式
    ├── errors/
    ├── health/
...
drizzle/                              # 0000–0009 SQL migrations 与 snapshots
scripts/                              # 本地服务、迁移配套、用户创建、集成烟测
tests/                                # Vitest 单元/模块测试
```
...
M0–M5 均已在当前仓库落地；以下保留为实现边界和回归验收分组。
...
以下问题不阻塞本地运行，但会影响正式上线配置：

1. 正式域名、TLS 终止方式和反向代理。
2. 部署平台、生产 PostgreSQL 和 S3 兼容对象存储供应商及区域。
3. 数据库备份、对象生命周期、恢复演练和多实例发布策略。
4. 是否补充找回密码邮件、管理员邀请和账号生命周期流程。

当前实现：界面和数据模型支持多用户；认证采用邮箱密码与数据库会话，公开注册关闭；默认时区为 Asia/Shanghai；文件保持私有。生产构建与运行必须共享 `NEXT_SERVER_ACTIONS_ENCRYPTION_KEY`，每次发布必须设置唯一 `DEPLOYMENT_VERSION`。
