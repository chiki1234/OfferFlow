...
Browser
  ...
  |-- Server Actions -----------+
  |                              |
  `-- Future AI Route Handlers -+--> JobWorkflow.execute(command)
                                 `--> InterviewKnowledge.execute(command)
                                                    |
...
### 4.2 InterviewKnowledge

负责 FAQ Block 解析、校验、批量绑定和确认入库。

```ts
interface InterviewKnowledge {
  execute(
...
FAQ 解析分为两个动作：`parse_faq_blocks` 不写库；`commit_faq_batch` 在用户修正并确认后写库。
...
约束：用户不直接配置 `kind`，而是逐条必选是否绑定经历。绑定时必须有 `experience_id`、`category` 必须为空，并自动派生 `kind=experience`；未绑定时 `experience_id` 必须为空、分类可选，并自动派生 `kind=general`。只有未绑定且分类为空的 FAQ 标记为待完善。JobTrack 通过 source Interview 推导，不在 FAQ 中重复保存。
...
faqs(user_id, experience_id, category)
```
