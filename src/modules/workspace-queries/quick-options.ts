...
export async function getQuickActionOptions(userId: string) {
  ...
  const [jobs, interviewRows, unboundTaskRows] = await Promise.all([
    db.select({ id: jobTracks.id, companyName: jobTracks.companyName, department: jobTracks.department, roleName: jobTracks.roleName })
  ...
  return { jobs, interviews: interviewRows, unboundTasks: unboundTaskRows.map((task) => ({ ...task, startAt: task.startAt?.toISOString() ?? null, endAt: task.endAt?.toISOString() ?? null, deadlineAt: task.deadlineAt?.toISOString() ?? null })) };
}
