...
import { DeleteJobButton } from "../jobs-page-actions";
...
export async function JobDetailContent({ id }: { id: string }) {
  ...
  return <div className="page-stack job-detail-dialog-content">
    <header className="page-heading detail-heading">
      <div>
        <p className="eyebrow">{lifecycleLabel(view.jobTrack.lifecycle)}</p>
        <h1>{view.jobTrack.companyName}{view.jobTrack.department ? ` · ${view.jobTrack.department}` : ""} | {view.jobTrack.roleName}</h1>{view.jobTrack.preferenceRank && <p>{preferenceLabel(view.jobTrack.preferenceRank)}</p>}
      </div>
      <DeleteJobButton detail jobTrackId={id} jobLabel={`${view.jobTrack.companyName} · ${view.jobTrack.roleName}`} token={randomUUID()} />
    ...
    <div className="detail-main job-detail-content">
      ...
      <section id="job-description" className="surface-card detail-section job-description-card">
        <div className="section-title-row">
          <h2>岗位 JD</h2>
          <div className="section-actions">
            <EditJobButton jobTrackId={id} />
      ...
      <section className="surface-card detail-section">
        ...
        <div className="milestone-list">
          {view.assessments.map((item) => <article className="milestone-item" id={`assessment-${item.id}`} key={item.id}>
            <div><strong>{item.title}</strong>{item.assessmentUrl && <a className="external-event-link" href={item.assessmentUrl} target="_blank" rel="noreferrer">测评链接 ↗</a>}<p>{item.timing.type === "deadline" ? `截止 ${formatDateTime(item.timing.deadlineAt)}` : `${formatDateTime(item.timing.startAt)} – ${formatDateTime(item.timing.endAt)}`}</p></div>
            ...
            <div className="milestone-actions">
              ...
              <ActionDialog label="编辑" title="编辑测评 / 笔试"><AssessmentEditor jobTrackId={id} assessment={item} /></ActionDialog>
          ...
          {view.interviews.map((item) => <article className="milestone-item" key={item.id}>
            <Link href={`/interviews/${item.id}`}><strong>{[item.roundLabel, item.interviewType].filter(Boolean).join(" · ")}</strong><p>{formatInterviewTiming(item.timing)}</p></Link>
            <span className={`status-pill ${item.status}`}>{item.reviewedAt ? "已复盘" : item.occurredAt ? "已发生" : statusLabel(item.status)}</span>
            <div className="milestone-actions">
              {item.status === "scheduled" && !item.reviewedAt && <><StatusActionButton intent="cancel_interview" jobTrackId={id} label="取消" subjectId={item.id} token={randomUUID()} />{(item.timing.type === "deadline" || new Date(timingStart(item.timing)) <= new Date()) && !item.occurredAt && <StatusActionButton intent="confirm_interview_occurred" jobTrackId={id} label={item.timing.type === "deadline" ? "完成" : "确认已发生"} subjectId={item.id} token={randomUUID()} />}{new Date(timingEnd(item.timing)) <= new Date() && (item.timing.type === "fixed_slot" || Boolean(item.occurredAt)) && <StatusActionButton intent="complete_interview_review" jobTrackId={id} label="完成复盘" subjectId={item.id} token={randomUUID()} />}</>}
...
function timingStart(timing: import("@/modules/job-workflow/interface").InterviewTiming) { return timing.type === "deadline" ? timing.deadlineAt : timing.startAt; }
