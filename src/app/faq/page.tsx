    ...
    <div className="knowledge-layout">
      <section className="knowledge-main">
        {activeView === "questions" ? <>
          ...
          return <article className="surface-card faq-card" key={faq.id}><div className={settingsComplete ? "faq-card-meta" : "faq-card-meta incomplete"}><span>{settingLabel}</span><FaqFrequency count={faq.frequency} /></div><h3>{faq.question}</h3><FaqAnswerDisclosure answer={faq.answer} question={faq.question} /><FaqSources sources={faq.sources} /><FaqEditor faq={faq} experiences={library.experiences} faqCategories={library.faqCategories} incomplete={!settingsComplete} /></article>;
...
function buildViewHref(filters: { query: string; binding: string | null; experienceId: string | null; category: string | null; settings: string | null }, view: "questions" | "experiences") {
