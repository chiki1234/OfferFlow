    ...
    <div className="knowledge-layout">
      <section className="knowledge-main">
        {activeView === "questions" ? <>
          ...
          {library.faqs.length ? <div className="faq-card-list">{library.faqs.map((faq) => <FaqCard key={faq.id} faq={faq} experiences={library.experiences} faqCategories={library.faqCategories} />)}</div> : <div className="surface-card empty-state jobs-empty"><span className="empty-icon"><CircleHelp size={22} /></span><div><strong>{library.totalFaqCount ? "没有匹配的问题" : "知识库还没有问题"}</strong><p>{library.totalFaqCount ? "调整搜索词或筛选条件后再试。" : "完成面试后，可以一次粘贴多个 FAQ Block。"}</p></div></div>}
